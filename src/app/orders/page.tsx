...
const STATUS_CONFIG = {
  ...
  CANCELLED: { label: 'Cancelled',    icon: XCircle,      color: 'text-[#E84545]',  bg: 'bg-red-50',    border: 'border-red-200'    },
};
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F0]">
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        <div className="mb-8">
          <p className="text-[#E84545] text-[10px] font-bold tracking-widest uppercase mb-1">Account</p>
          <h1 className="text-2xl font-extrabold text-[#111111] uppercase tracking-wide flex items-center gap-3">
            My Orders
          </h1>
        </div>
        ...
        {error ? (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <h2 className="text-lg font-extrabold text-[#111111] uppercase tracking-wide mb-2">No orders yet</h2>
              ...
              return (
                <motion.div key={order.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                  <Link href={`/orders/${order.id}`}>
                    <div className="bg-white border border-[#E5E7EB] p-5 hover:shadow-md transition-all cursor-pointer group rounded-sm">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-extrabold text-[#111111] text-sm uppercase tracking-wide">Order {orderLabel}</p>
                        ...
                        <div className="flex items-center gap-3">
                          {methodLabel && (
                            <span className="text-[9px] font-bold text-[#6B7280] bg-[#F3F4F6] px-2 py-1 rounded-sm uppercase tracking-wider">
                              {methodLabel}
                            </span>
                          )}
                          <div className={`flex items-center gap-1.5 px-3 py-1.5 border rounded-sm ${cfg.bg} ${cfg.border}`}>
                            <Icon className={`w-3.5 h-3.5 ${cfg.color}`} />
                            <span className={`text-[10px] font-bold ${cfg.color} uppercase tracking-wider`}>{cfg.label}</span>
                          </div>
                          <ChevronRight className="w-5 h-5 text-[#D1D5DB] group-hover:text-[#E84545] transition-colors" />
                        </div>
                      </div>
                      <div className="mt-3 pt-3 border-t border-[#F3F4F6] flex justify-between items-center">
                        ...
                        <p className="font-extrabold text-[#111111]">{formatPrice(order.totalAmount)}</p>
                      </div>
                    </div>
                  </Link>
                </motion.div>
